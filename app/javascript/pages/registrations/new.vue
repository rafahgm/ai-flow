...
<template>
    ...
    <div class="min-h-screen flex items-center justify-center bg-surface-50 px-40">
        <div class="w-full max-w-md bg-white rounded-2xl shadow p-6">
            <h1 class="text-2xl font-semibold mb-1">Criar conta</h1>
            ...
            <form class="flex flex-col gap-4" @submit.prevent="submit">
                <div class="flex flex-col gap-1">
                    <InputText id="nome" v-model="form.name" fluid placeholder="Nome" />
                ...
                <div class="flex flex-col gap-1">
                    <Password id="password_confirmation" placeholder="Confirmação da senha" v-model="form.password_confirmation" fluid toggle-mask :feedback="false" />
                ...
                <Button type="submit" label="Criar conta" :loading="form.processing" fluid />
            </form>

            <div class="mt-6 text-sm text-center">
                <Link href="/login" class="text-primary no-underline">Fazer login</Link>
            </div>
        </div>
    </div>
</template>